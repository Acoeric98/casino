...
*/
"use strict";
console.log("✅ LOADED: poker_restapi_final.js");
var START_DATE;
var NUM_ROUNDS;
var STOP_AUTOPLAY = 0;
var RUN_EM = 0;
...
async function init () {
  ...
  const bal = parseInt(await res.text());
  console.log(bal)
  STARTING_BANKROLL = 500
  if (typeof players !== 'undefined' && players[0]) players[0].stack = bal;
  player.stack = bal;
  startingStack = bal;
  showBalance();
...
async function new_game_continues (req_no_opponents) {
  const res = await fetch("../api/get_balance.php");
  const bal = parseInt(await res.text());
  STARTING_BANKROLL = bal;


  
  var my_players = [
    new player("Plavdik Szabolcs", 0, "", "", "", 0, 0),
  ...
  var player_name = getLocalStorage("playername") || "You";

  // 🔥 REST balance-al létrehozzuk saját magunkat
  players[0] = new player(player_name, bal, "", "", "", 0, 0);
  ...
  write_player(0, 1, 1); // 🖥️ frissítsük a .chips értéket

  console.log("💰 PLAYER 0 bankroll right before round:", players[0].bankroll);

  new_round();
  console.log("PLAYER 0 PROFIT:", players[0].bankroll - startingStack);
}
...
async function handle_end_of_round () {
  ...
  if (typeof players !== "undefined" && players[0]) {
    const profit = players[0].bankroll - startingStack;

    if (profit > 0) {
      await fetch("../api/update_balance.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ delta: profit })
      });
      console.log("✅ REST nyeremény hozzáadva:", profit);
    }
  }
  const res = await fetch("../api/get_balance.php");
  const bal = parseInt(await res.text());
  console.log(" Új egyenleg: " + bal)
...
//                     " pot_remainder: " + pot_remainder);
  ...
  if (typeof players !== "undefined" && !players[0].profitHandled) {
    const profit = players[0].bankroll - startingStack;
    ...
    if (profit !== 0) {
      ...
      if (profit > 0) {
        ...
        console.log("❌ REST veszteség levonva:", profit);
      }
    }
  
    players[0].profitHandled = true;
  }
...
async function showBalance() {
  ...
  if (div) div.textContent = bal;
}


function start_poker() {
  showBalance();
  init();

  // Biztonság kedvéért újrarendereljük a játékost REST balance-szal
  setTimeout(() => {
    if (typeof players !== "undefined" && players[0]) {
      console.log("DEBUG bankroll érték:", players[0].bankroll);
      write_player(0, 1, 1);
    }
  }, 1000);
